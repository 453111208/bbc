 ...
 * @license 
 */
class sysinfo_article_marketArticle
{
    //保存行情
    public function saveContent($data)
    {
        $nodeMdl = app::get('sysinfo')->model('marketNode');
        $articleMdl = app::get('sysinfo')->model('marketArticle');
    ...
    private function __checkdata($contentData)
    {
        ...
        $validator = validator::make(
            ...
            ['title' => '行情标题不能为空'      , 'node_id' => '行情节点id不能为空']
        );
        if ($validator->fails())
        {
    ...
    public function getArticleInfo($params)
    {
        ...
        {
            throw new \LogicException('行情id不能为空!');
        }
        if(!$params['fields'])
        {
            $params['fields'] = '*';
