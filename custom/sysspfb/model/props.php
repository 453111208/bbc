...
class sysspfb_mdl_props extends dbeav_model{
    ...
    public function savePropValues($propValue, $propId, &$msg)
    {
        ...
        foreach( (array)$propValue as $key=>$propValueRow )
        {
            ...
            if( !$this->propValuesModel->save($savePropValue) ){
                $msg = app::get('sysspfb')->_("属性值{$propValueRow['prop_value']}保存失败");
                return false;
            }
            $orderSort++;
        }
    ...
    public function doDelete($propId)
    {
        ...
        if($relProp)
        {
            $msg = app::get('sysspfb')->_('属性已经与类目绑定，不可删除');
            throw new \LogicException($msg);
            return false;
        }
