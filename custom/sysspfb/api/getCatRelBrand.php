<?php
class sysspfb_api_getCatRelBrand{
    public $apiDescription = "获取类目关联的品牌";
    public function getParams()
    {
        $return['params'] = array(
            'shop_id' => ['type'=>'string','valid'=>'', 'description'=>'签约企业id','example'=>'3','default'=>''],
    ...
    public function getData($params)
    {
        ...
        if(!$catId)
        {
            throw new \LogicException(app::get('sysspfb')->_('参数cat_id有误'));
        }
        ...
        if(!$cat || !$cat['level'])
        {
            throw new \LogicException(app::get('sysspfb')->_('参数类目cat_id='.$catId.'不存在'));
        }

        if($shopId)
        {
            $shopAuthorize = app::get('sysspfb')->rpcCall('shop.authorize.catbrandids.get',array('shop_id'=>$shopId));
            if(!isset($shopAuthorize[$shopId])){
                throw new \LogicException(app::get('sysspfb')->_('不存在该企业shop_id='.$shopId));
            }
            ...
            if($cat['level'] == 3 && (in_array($arrPath[1],$authorize['cat']) || $shopType == "self"))
            {
                ...
                if(!$relBrand)
                {
                    throw new \LogicException(app::get('sysspfb')->_('该类目下无关联品牌'));
                }
            ...
            {
                throw new \LogicException(app::get('sysspfb')->_('不存在shop_id='.$shopId.'的企业'));
            }
        }
        ...
        if(!$lv3CatIds)
        {
            throw new \LogicException(app::get('sysspfb')->_('参数cat_id='.$catId.'有误'));
        }
    ...
    private function __getRelBrand($catIds)
    {
        ...
        $objMdlRelBrand = app::get('sysspfb')->model('cat_rel_brand');
