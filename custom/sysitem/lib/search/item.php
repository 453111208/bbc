...
class sysitem_search_item  {
    ...
    public function count($filter)
    {
        ...
        return $qb->select('count(*) as _count')
    ...
    //检查传入字段在哪个表中
    private function __colsByTable($cols)
    {
    ...
    private function __getSelectColumns($columns)
    {
        ...
        {
            $table = $this->__colsByTable($column);
            if( !$table )
            {
                $returnCols[] = $column;
            }
    ...
    public function getList($cols='*', $filter=array(), $offset=0, $limit=-1, $orderBy=null, $groupBy='')
    {
        ...
        if ($orderBy)
        {
            ...
            array_map(function($o) use (&$qb){
                ...
                $sort = $this->__colsByTable($sort) ? $this->__colsByTable($sort).'.'.$sort : $sort;
