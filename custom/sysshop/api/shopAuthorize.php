...
class sysshop_api_shopAuthorize{
     ...
     * 接口作用说明
     */
    public $apiDescription = '获取企业签约的类目和品牌信息(id集)和企业类型';
    ...
    public function getParams()
    {
        $return['params'] = array(
            ...
            'fields' => ['type'=>'field_list','valid'=>'','description'=>'要获取企业签约的类目和品牌字段集','default'=>'cat.cat_id,cat,cat_name,brand.brand_id,brand.brand_name','example'=>'cat.cat_id,cat,cat_name,brand.brand_id,brand.brand_name'],
        );
    ...
    public function getCatBrand($params)
    {
        ...
        $catmdl=app::get('syscategory')->model("cat");

        $result[$filter['shop_id']] = $objMdlShop->getRow('shop_type',$filter);
        $cats = $catmdl->getList($catRows,array("level"=>1));
        foreach($cats as $cat)
        {
