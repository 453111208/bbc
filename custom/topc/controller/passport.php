...
class topc_ctl_passport extends topc_controller
{
       ...
       public function signupc()
    {
        ...
        return $this->page('topc/passport/signup/signupc.html', $pagedata);
    }
    ...
    public function create()
    {
        ...
        //     }
            ...
            $sql="UPDATE sysshop_seller set seller_type=3 where seller_id in  (select seller_id from sysshop_account where login_account = '".$userInfo['account']."' )";
     ...
     public function checkEmailAccount()
    {
        ...
        $signAccount = utils::_filter_input(input::get());
        $email = $signAccount['pam_user']['email'];
        $validator = validator::make(
        ...
        try
        {
            $data = app::get("sysshop")->model("seller")->getRow("*",array("email"=>$email));
            if($data)
            {
                throw new \LogicException('该邮箱已被使用');
            }
            //  if( is_numeric($loginName) )
            // {
            // throw new \LogicException('登录账号不能全为数字');
