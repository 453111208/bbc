...
class syscategory_ctl_admin_props extends desktop_controller {
    ...
    function selPropDialog() {
        $oProp = app::get('syscategory')->model('item_prop');
        //修改选择的属性 by litong
        $aProp = $oProp->getList('item_prop_id,item_prop_name',null,0,-1);
        $pagedata['props'] = $aProp;
        return view::make('syscategory/admin/itemprop/item_prop_select.html', $pagedata);
    }

    function previewProp(){
        $objMdlProps = app::get('syscategory')->model('item_prop');
        $pagedata['prop'] = $objMdlProps->dump( $_POST['item_prop_id'], '*',array('item_prop_name'=>array('*')));
    $a= $pagedata['prop'];
