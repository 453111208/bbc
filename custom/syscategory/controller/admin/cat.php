...
class syscategory_ctl_admin_cat extends desktop_controller {
    ...
    public function index()
    {
        ...
        if($tree)
        {
            foreach($tree as $k=>$v)
            {
                ...
                $cat_id=$v["cat_id"];
                $cat_info=app::get('syscategory')->model('cat')->getRow("*",array("cat_id"=>$cat_id));
                $tree[$k]["is_bz"]=$cat_info["is_bz"];
    ...
    public function edit($nCatId, $is_leaf=0)
    {
        ...
        $catInfo = $objMdlCat->getRow('cat_path, level, cat_name, cat_logo, cat_id, parent_id, guarantee_money, platform_fee, cat_service_rates, order_sort,cat_template,is_bz', array('cat_id'=>$nCatId));

        // 组织上级分类数据，1级分类显示无，2级分类显示1级分类,3级分类则显示对应的2级和1级分类
        $catList[0] = array('cat_id'=>0, 'cat_name'=>app::get('syscategory')->_('----无----'), 'step'=>1);
        if($catInfo['level']=='2')
        {
            $level1CatInfo = $objMdlCat->getRow('cat_path, level, cat_name, cat_id, parent_id, guarantee_money, platform_fee, cat_service_rates, order_sort,is_bz', array('cat_id'=>$catInfo['parent_id']));
            $catList[1] = array('cat_id'=>$level1CatInfo['cat_id'], 'cat_name'=>$level1CatInfo['cat_name'], 'step'=>1);
        }
        if($catInfo['level']=='3')
        {
            $level2CatInfo = $objMdlCat->getRow('cat_path, level, cat_name, cat_id, parent_id, guarantee_money, platform_fee, cat_service_rates, order_sort,is_bz', array('cat_id'=>$catInfo['parent_id']));
            $level1CatInfo = $objMdlCat->getRow('cat_path, level, cat_name, cat_id, parent_id, guarantee_money, platform_fee, cat_service_rates, order_sort,is_bz', array('cat_id'=>$level2CatInfo['parent_id']));
            $catList[1] = array('cat_id'=>$level1CatInfo['cat_id'], 'cat_name'=>$level1CatInfo['cat_name'], 'step'=>1);
        ...
        $pagedata['cat']['is_bz'] = $catInfo['is_bz'];
    ...
    public function save()
    {
       ...
       if($postData['cat']['parent_id']){
        $catInfobz=$objMdlCat->getRow("is_bz",array('cat_id'=>$postData['cat']['parent_id']));
        $postData['cat']["is_bz"]= $catInfobz["is_bz"];
        }
        else{
            if($postData['cat']['cat_id']){
                $lv2= $objMdlCat->getList('*', array("parent_id"=>$postData["cat"]["cat_id"]));
                if($lv2){
                    foreach ($lv2 as $key => $value) {
                        $value["is_bz"]= $postData['cat']["is_bz"];
                        $objMdlCat->save($value);
                        $lv3= $objMdlCat->getList('*', array("parent_id"=>$value["cat_id"]));
                         if($lv3){
                            foreach ( $lv3 as $key2 => $value2) {
                                 $value2["is_bz"]= $postData['cat']["is_bz"];
                                 $objMdlCat->save($value2);
                            }
                         }
                    }
                }
    ...
    public function relProp($cat_id)
    {
        ...
        {
            $props[$v['prop_id']] = $v;
            $propsInfo = $objMdlProps->getRow('item_prop_Id,item_prop_name',array('item_prop_id'=>$v['prop_id']));
            $props[$v['prop_id']]['prop'] = $propsInfo;
        }
        $aaa= $props;
        $bbb=$aaa[1]; 
        $pagedata['props'] = $props;
        $ccc=$pagedata['props'][0]["item_prop_id"];
        $pagedata['cat_id'] = $cat_id;
    ...
    public function saveRelProp()
    {
        ...
        $prop = array();
        $propIds = array();
        $aaaa1 = $postData['prop']['item_prop_id'][0];
        $aaaa2 = $postData['prop']['item_prop_id'][1];
        
        $bbbb=$postData['cat_id'];
        foreach( $postData['prop']['item_prop_id'] as $k => $aProp )
        {
            $propIds[] = $aProp;
            $prop[] = array(
                'prop_id' => $aProp,
