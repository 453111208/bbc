...
class sysshoppubt_ctl_tenderule extends desktop_controller{
    ...
    public function create($tenderrule_id){
        ...
        if($tenderrule_id !=""){
            ...
            $tenderule_detail = app::get('sysshoppubt')->model('detail');
            $row = $tenderule_model->getRow('*',array('tenderrule_id'=>$tenderrule_id)); 
            $rowdetail = $tenderule_detail->getList('*',array('tenderrule_id'=>$tenderrule_id)); 
            $pagedata['row'] =  $row;
            $pagedata['rowdetail'] =  $rowdetail;
        }
    ...
    public function save(){
            ...
            foreach ($data as $key => $value) {
             ...
             if($newdata['tenderrule_id'] != ""){
                ...
                if($keyinfo)$newdata['have_detail'] = 1;else $newdata['have_detail'] = 0;
