...
class sysshoppubt_finder_biddings{
    ...
    public function detail_bidding($row){
        ...
        /*$oItem = kernel::single("sysshoppubt_mdl_sprodrelease");*/
        $uc = $oItems->getList('uniqid',array('bidding_id' => $row));
        $pagedata['deliveryaddr'] = $oItem->getList('*',array('uniqid' => $uc[0]['uniqid'],'def_addr'=>1));
        //return 'detail';
    ...
    public function detail_bidding1($row){
        $oItem = app::get('sysshoppubt')->model('tradeorder');
        $biddings = $oItem->getList('*',array('bidding_id' => $row));
        $pagedata['tradeorder'] = $biddings;
