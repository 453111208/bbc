...
class sysuser_finder_user {
    ...
    public function __construct($app)
    {
        ...
        $this->detail_premoney = app::get('sysuser')->_('会员预存款');
        $this->detail_changemoney = app::get('sysuser')->_('存款变动详细列表');
           
    }


public function detail_changemoney($row)
    {
        $reco = app::get('sysuser')->model('moneyreco');
        $pagedata['items'] = $reco->getList('*',array('user_id'=>$row),0,-1,'create_time desc');
        return view::make('sysuser/admin/user/changemoney.html', $pagedata)->render();
    }
    ...
    public function _column_editbutton_2($row)
    {
        ...
        public function detail_premoney($row)
    {
        $objMdlUser = app::get('sysuser')->model('user');
        $money = $objMdlUser->getRow('*',array('user_id'=>$row));
        $pagedata['userid'] = $money['user_id'];
        $pagedata['premoney'] = $money['hjadvance'];
        return view::make('sysuser/admin/user/premoney.html', $pagedata)->render();
    }
    ...
    public function column_uname(&$colList, $list)
    {
        ...
        foreach($list as $k=>$row)
        {
            $userInfoList = app::get('sysuser')->model('account')->getRow('login_account', array('user_id'=>$row["user_id"]));
            $colList[$k] = $userInfoList['login_account'];
        }
    }
    ...
    //     $paminfo = $saveItem->getRow('hjadvance',array('user_id'=>$row));
    //     $pagedata['advance'] = $paminfo['hjadvance'];
