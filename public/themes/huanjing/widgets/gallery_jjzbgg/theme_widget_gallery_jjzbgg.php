...
function theme_widget_gallery_jjzbgg(&$setting)
{
    $noticeids = "";
    foreach ($setting['jjzbggList'] as $key => $value) {
        $noticeids.=",'".$value."'";
    }
    $noticeids = substr($noticeids,1);
    if($noticeids){ 
    } else{
        $noticeids=-1;
    }
...
SELECT
            ...
            GROUP BY
                std.tender_id, std.shop_id, std.shop_name, std.trading_title, ssp.shop_area) t1 WHERE t1.bidorten_id IN (".$noticeids.") ORDER BY t1.stop_time DESC";
    ...
    foreach ($sprodreleaseList as $key => $value) {
        ...
        $bidorten_id_arr=explode(";", $bidorten_id);
