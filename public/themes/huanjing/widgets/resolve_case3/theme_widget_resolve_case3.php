...
function theme_widget_resolve_case3(&$setting){
    foreach($setting['resolve_case3'] as $key=>$value)
    {
        $essay_id =$value;
        ...
        $_return['resolve_case3'][$key]["data_id"]=$value;
        $_return['resolve_case3'][$key]["data"]=$essaylist[0];
        $_return["secondSort"]=$essaylist[0]['second_sort'];
        $date=explode("年",$essaylist[0]['date']);
        $_return['resolve_case3'][$key]["date"]=$date[1];
        //var_dump($_return['resolve_case3'][$key]);
    }
    return $_return;
}
?>
