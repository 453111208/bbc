var TaskRunner = new Class({
    ...
    createFormData:function(ipt){
        ...
        var options=this.options,fdoc=document.createDocumentFragment(),
            data=ipt?ipt:$$(options.dataClass);
        if(data&&data.length)
    ...
    loader:function(){
        ...
        if(this.showStep)this.showStep.set('text',this.step);
        if(this.appName)this.appName.set('text',this.items[this.prestep].get('appname'));
    ...
    check:function(iframe){
        ...
        if(body.innerText){
            var messageText=body.innerText;
        }else{
            var messageText=body.textContent;
        }
...
var validate = function(_form,match){
    ...
    var _return = formElements.every(function(element){
         ...
         return vtype.split('&&').every(function(key){
                ...
                if(validateMap[key][1](element,element.get('value'))){
