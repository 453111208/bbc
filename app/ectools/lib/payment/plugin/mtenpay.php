 ...
 */
final class ectools_payment_plugin_mtenpay extends ectools_payment_app implements ectools_interface_payment_app {
    public $name = '手机财付通';
    ...
    public function __construct($app){
		...
		$this->notify_url = kernel::openapi_url('openapi.ectools_payment/parse/' . $this->app->app_id . '/ectools_payment_plugin_mtenpay_server', 'callback');
		...
		$this->callback_url = kernel::openapi_url('openapi.ectools_payment/parse/' . $this->app->app_id . '/ectools_payment_plugin_mtenpay', 'callback');
    ...
    public function setting(){
        return array(
            ...
            'mer_id'=>array(
                'title'=>app::get('ectools')->_('商户号(bargainor)'),
                'type'=>'string',
                'validate_type' => 'required',
            ),
            'mer_key'=>array(
                'title'=>app::get('ectools')->_('商户签名(key)'),
                'type'=>'string',
                'validate_type' => 'required',
            ),
            'support_cur'=>array(
    ...
    */
    public function callback(&$recv)
    {
        ...
        if($this->is_return_vaild($recv,$mer_key,$this->sec_id)){
            ...
            $ret['account'] = $mer_id;
            $ret['bank'] = app::get('ectoole')->_('手机财付通');
            $ret['pay_account'] = app::get('ectoole')->_('付款帐号');
            $ret['currency'] = 'CNY';
            $ret['money'] = $money;
            $ret['paycost'] = '0.000';
            $ret['cur_money'] = $money;
    ...
    public function intro(){
        return app::get('ectools')->_('财付通为手机支付提供简单高效的解决方案。用户通过手机在财付通合作网站上进行订单确认支付后，将接收财付通发送的验证码，回复验证码并确认即可完成订单支付。');
    }
