...
class desktop_ctl_backup extends desktop_controller{
    ...
    function index(){
        ...
        $pagedata['backup'] = 'current';
        kernel::single("desktop_ctl_data")->index();
        return $this->page('desktop/system/backup/backup.html', $pagedata);
    }
    ...
    function backup_sdf(){
        ...
        if(!$oBackup->start_backup_sdf($params,$nexturl)){
           ...
           echo '{success:"'.app::get('desktop')->_('备份完成').'",nexturl:"?app=desktop&ctl=backup&act=getFile&file=multibak_'.$params['dirname'].'.zip"}';
