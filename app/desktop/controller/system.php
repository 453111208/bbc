...
class desktop_ctl_system extends desktop_controller{
    ...
    function set_title(){
        ...
        echo '<h4 class="head-title" >'.app::get('desktop')->_('标题设置').'</h4>';
            $html = view::ui()->form_start(array('action'=>'?ctl=system&act=set_title','method'=>'post'));
            $background_title = $this->app->getConf('background.title');
            $html .= view::ui()->form_input(array('title'=>app::get('desktop')->_('标题：'),'name'=>'background.title','tab'=>'后台设置','value'=>$background_title,'vtype'=>'required'));
