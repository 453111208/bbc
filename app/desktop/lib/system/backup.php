...
class desktop_system_backup {
    ...
    public function start_backup_sdf(&$params,&$nexturl){ 
        ...
        if($app){
            $nexturl = "?app=desktop&ctl=backup&act=backup_sdf&appname=$app&dirname=$dirname";
            $params['app'] = $app;
        } else {
    ...
    private function create_table( $app,$model,$o,$dbschema_file,$createtable=true ) {
        ...
        $dbtable = kernel::single('base_application_dbtable');
        
        $dbtable = $dbtable->detect($app, $model);
        $tableName = $dbtable->real_table_name();
        $dbtable->refreshLoad($dbschema_file, $tableName);

        if( $createtable )
        {
            $db = app::get($app)->database();
            if ($db->getSchemaManager()->tablesExist($tableName))
            {
                $db->getSchemaManager()->dropTable($tableName);
            }
            $db->exec($dbtable->getCreateTableSql());
        } else {
        }
    }
