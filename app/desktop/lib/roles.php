...
class desktop_roles{
    ...
    function __construct($app)
    {
        ...
        $this->model = $this->app->model('roles');
    }
    ...
    function get_permission_per($menu_id,$wg)
    {
        ...
        foreach($aMenus as $val)
        {
            $sdf = $menus->dump(array('menu_type' => 'permission','display'=>1,'permission' => $val)); //permission 加显示判断  by shuhanbing
    ...
    {
        $menus = $this->app->model('menus');
        $aPer = $menus->getList('*',array('menu_type' => 'permission','disabled' => 0,'display'=>1)); //permission 加显示判断  by shuhanbing
    ...
    function get_others($wg,&$othersflg=0)
    {
        ...
        $aData = array();
        $arr_per = $menu->getList('*',array('menu_type'=>'permission','disabled'=>0,'display'=>1));
