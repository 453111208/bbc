...
class importexport_tasks_runimport extends base_task_abstract implements base_interface_task{
    ...
    public function exec($params=null){
        ...
        while( !feof($file) ){
            ...
            if($contents){
                ...
                if( $msg['error'] ){
                    ...
                    $error = '';
                    if( $offset === 0 ) $error = $filetypeObj->fileHeader();
                    $offset++;
                    ...
                    $flag = $o->save($rs);
                    if( !$flag ){
                        $msg = app::get('importexport')->_('数据库插入错误');
                        $taskModel->update(array('status'=>7,'message'=>$msg),array('key'=>$params['key']));
                        exit;
                    }else{
                        //如果是有错误过则是部分导入
        ...
        if( $offset )
        {
            ...
            if( $fileFoot )
            {
                $this->write_error_file($fileFoot);
                $policyObj->push(array('local'=>$this->error_local_file,'remote'=>$this->error_remote_file,'resume'=>$this->error_ftell));
            }
        }
