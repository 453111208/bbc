...
class sysspfb_mdl_brand extends dbeav_model {
    ...
    public function doDelete($brandId)
    {
        ...
        $objMdlCatBrand = app::get('sysspfb')->model('cat_rel_brand');
        $objMdlShopBrand = app::get('sysspfb')->model('shop_rel_brand');
        $objMdlBrand = app::get('sysspfb')->model('brand');

        $CatInfo = $objMdlCatBrand->getList('cat_id,brand_id',array('brand_id'=>$brandId));
        if($CatInfo)
        {
            $msg = app::get('sysspfb')->_('品牌关联分类，不可被删除');
        ...
        if($shopBrandInfo)
        {
            $msg = app::get('sysspfb')->_('该品牌被店铺签约，不可被删除');
            throw new \LogicException($msg);
            return false;
        }
