...
class sysspfb_ctl_admin_cat extends desktop_controller {
    ...
    {
        $objMdlCat = app::get('sysspfb')->model('cat');
        ...
        if($tree)
        {
            foreach($tree as $k=>$v)
            {
                $tree[$k]['link'] = array('cat_id' => array(
                    'v' => $v['cat_id'],
                    't' => app::get('sysspfb')->_('商品类别').app::get('sysspfb')->_('是').$v['cat_name']
                ));
            }
        }

        $pagedata['tree'] = $tree;
        return $this->page('sysspfb/admin/category/map.html', $pagedata);
    }
    ...
    public function leaf()
    {
        ...
        return $this->finder('sysspfb_mdl_cat', array(
            'title' => app::get('sysspfb')->_('三级分类('.$level1['cat_name'].'->'.$level2['cat_name'].')'),
    ...
    public function add($nCatId = 0)
    {
        ...
        $catList[0] = array('cat_id'=>0, 'cat_name'=>app::get('sysspfb')->_('----无----'), 'step'=>1);
        ...
        return view::make('sysspfb/admin/category/info.html', $pagedata);
    }
    ...
    public function save()
    {
        ...
        if($postData['is_leaf'])
        {
            $this->begin('?app=sysspfb&ctl=admin_cat&act=leaf&parent_id='.$postData['cat']['parent_id']);
        }
        else
        {
            $this->begin('?app=sysspfb&ctl=admin_cat&act=index');
        }
        ...
        {
            $this->end(false, app::get('sysspfb')->_('同级分类下名称不能重复!'));
        }
        else
        {
            ...
            else
            {
                ...
                $this->end(false, app::get('sysspfb')->_('保存失败'));
            }
        }
    ...
    public function toRemove($nCatId)
    {
        ...
        {
            $this->begin('?app=sysspfb&ctl=admin_cat&act=leaf&parent_id='.$parentId);
        }
        else
        {
            $this->begin('?app=sysspfb&ctl=admin_cat&act=index');
        ...
        try
        {
            ...
            $flag = app::get('sysspfb')->rpcCall('category.cat.remove',$params);
        ...
        $this->end(true, $catInfo['cat_name'].app::get('sysspfb')->_('已删除'));
    }
    ...
    public function updateSort()
    {
        ...
        $this->begin('?app=sysspfb&ctl=admin_cat&act=index');
        $objLibCat = kernel::single('sysspfb_data_cat');
        $objLibCat->updateSort($postData, $msg);
        $this->adminlog("更新商品类目排序", 1);
        $this->end(true, app::get('sysspfb')->_('更新排序操作完成'));
    }
    ...
    public function relBrand($cat_id)
    {
        ...
        $objMdlCatRelBrand = app::get('sysspfb')->model('cat_rel_brand');
        ...
        return $this->page('sysspfb/admin/cat_rel/cat_rel_brand.html', $pagedata);
    }
    ...
    public function saveRelBrand()
    {
        ...
        $this->begin('?app=sysspfb&ctl=admin_cat&act=leaf&parent_id='.$parent_id);
        if(!$postData['checkedBrand']) $postData['checkedBrand'] = null;
        $objCatRelBrand = app::get('sysspfb')->model('cat_rel_brand');
        ...
        $this->end(true, app::get('sysspfb')->_('操作成功'));
    }
    ...
    public function relProp($cat_id)
    {
        ...
        $objMdlCatRelProp = app::get('sysspfb')->model('cat_rel_prop');
        ...
        return $this->page('sysspfb/admin/cat_rel/cat_rel_prop.html', $pagedata);
    }
    ...
    public function relParam($cat_id)
    {
        ...
        return $this->page('sysspfb/admin/cat_rel/cat_rel_params.html', $pagedata);
    }
    ...
    public function saveRelParam()
    {
      ...
      /*  if( !$postData['params'] )
        ...
        foreach( $postData['params'] as $aParams )
        {
            if( !$aParams['name'] )
            {
                $msg = app::get('sysspfb')->_('请为参数表中参数组添加参数名');
                throw new \LogicException($msg);
                break;
            }
            $paramsItem = array();
            foreach( $aParams['name'] as $piKey => $piName )
            {
                if(!$piName)
                {
                    $msg = app::get('sysspfb')->_('请完成参数表中参数名');
                    throw new \LogicException($msg);
                    break 2;
                }
            ...
            if(!$aParams['group'])
            {
                $msg = app::get('sysspfb')->_('请完成参数表中参数组名称');
                throw new \LogicException($msg);
                break;
            }
