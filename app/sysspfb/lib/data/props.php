...
class sysspfb_data_props {
    ...
    {
        $this->propModel = app::get('sysspfb')->model('props');
    }
    ...
    public function add($data, &$msg)
    {
        ...
        {
            $msg = app::get('sysspfb')->_('属性名不能超过20个字符');
            return false;
        }
        if($data['is_def']){
        ...
        if( !$propId )
        {
            $msg = app::get('sysspfb')->_('属性保存失败');
            return false;
        }

        $flag = $this->propModel->savePropValues($data['prop_value'], $propId, $msg);
        if( $flag ) $msg = app::get('sysspfb')->_('属性保存成功');
        return $flag;
    }
    ...
    private function __check($data, &$msg)
    {
        ...
        foreach( (array)$data['prop_value'] as $propValue )
        {
            ...
            {
                $msg = app::get('sysspfb')->_('属性值不能为空');
                return false;
            }

            if( $this->__utf8Strlen($propValue['prop_value']) > 20 )
            {
                $msg = app::get('sysspfb')->_('属性值不能大于20个字');
                return false;
            }
            ...
            {
                $msg = app::get('sysspfb')->_("属性值{$propValue['prop_value']}重复");
                return false;
            }
            ...
            if( $i > 30 )
            {
                $msg = app::get('sysspfb')->_('属性值的条数不能大于30');
                return false;
            }
        }
        ...
        {
            $msg = app::get('sysspfb')->_('属性名称不能大于20个字');
            return false;
        }

        if ($this->__utf8Strlen($data['prop_memo']) > 20 )
        {
            $msg = app::get('sysspfb')->_('属性备注不能大于20个字');
            return false;
        }
    ...
    {
        $pic['prop_default_pic'] = app::get('sysspfb')->getConf('prop.default.pic');
        $pic['prop_image_height'] = app::get('sysspfb')->getConf('prop.image.height');
        $pic['prop_image_width'] = app::get('sysspfb')->getConf('prop.image.width');
        return $pic;
    }
    ...
    public function getNatureProps($catId)
    {
        ...
        $objMdlPropValues = app::get('sysspfb')->model('prop_values');
