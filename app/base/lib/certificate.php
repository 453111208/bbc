...
class base_certificate{
    ...
    static function register($data=null){
        ...
        logger::info("registe:".var_export($data, true));
        $result = $http->post($posturl,$data);
        //todo: 声称获取一个唯一iD，发给飞飞
        $result = json_decode($result,1);

        logger::info("registe:".var_export($result, true));
        if($result['res']=='succ'){
            if ($result['info'])
            {
                //1.3接口不再返回node_id信息
    ...
    static function get_certi_logo_url(){
        ...
        $result = $http->post( $posturl, $params);
        $tmp_res = json_decode($result, 1);
        logger::info("get_certi_logo_url:".var_export($tmp_res, true));
