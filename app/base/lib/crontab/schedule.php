...
class base_crontab_schedule{
    static public function trigger_one($cron_id){
        if ($cron = app::get('base')->model('crontab')->getRow('id, last', array('id' => $cron_id, 'enabled' => 1))){
            $now = time();
            if (($now - $cron['last'])<60) {
                //trigger_error(app::get('base')->_('1分钟之内不能重复执行'), E_USER_ERROR);
                throw new \LogicException(app::get('base')->_('1分钟之内不能重复执行'));
            }
    ...
    static public function __get_enabled_cronentries($filter=array()){
        //todo: 增加缓存
        $filter = array_merge(array('enabled' => 1),
                              $filter);
