    ...
    function init_iterator(){
        ...
        if($tmp_core_dir){
            ...
            if(defined('CUSTOM_CORE_DIR') && is_dir(CUSTOM_CORE_DIR.'/'.$this->target_app->app_id.'/'.$this->path)){
                ...
                $custom_core_dir = new CallbackFilterIterator($custom_core_dir, function($current){
            ...
            $params = $config_files;
            $core_dir = new CallbackFilterIterator($core_dir, function($current, $key, $iterator) use ($params){
