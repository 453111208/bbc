...
class sysshop_data_cat {
     ...
     * @brief 存储店铺分类数据
     *
     * @param array $data 店铺分类数据
     * @param string $msg 添加店铺分类成功或失败信息
     *
     * @return bool
     */
    public function addShopCat($data,$shopId)
    {
        ...
        foreach( (array)$data as $parentId => $val )
        {
            if( $val['cat_name'] )
            {
                ...
                if( $count > 100 )
                {
                    $msg = app::get('sysshop')->_('店铺分类总数量不能超过100个');
                ...
                if( !$parentId )
                {
                    $msg = app::get('sysshop')->_('店铺分类保存失败');
                    throw new \LogicException($msg);
                    return false;
                }
            }
            ...
            foreach( (array)$data['new'] as $parentId=>$cat )
            {
                foreach($cat as $col=>$row)
                {
                    if( is_array($row) )
                    {
                        ...
                        {
                            $msg = app::get('sysshop')->_("店铺分类名称{$row['cat_name']}不能重复");
                            throw new \LogicException($msg);
                        }
                    ...
                    elseif( $col == 'cat_name' )
                    {
                        ...
                        {
                            $msg = app::get('sysshop')->_("店铺分类名称{$row}不能重复");
                            throw new \LogicException($msg);
                        }
                        $catParentData[] = $row;
                    }
    ...
    private function __checkCatRepeat($checkData, &$catData, &$catParentData, $delCatId)
    {
        foreach( (array)$checkData as $catId=>$catRow )
        {
            ...
            if( $catRow['parent_id'] )
            {
                ...
                {
                    $msg = app::get('sysshop')->_("店铺分类名称{$catRow['cat_name']}不能重复");
                    throw new \LogicException($msg);
                }
    ...
    private function __checkCatName($catName)
    {
        if( empty($catName) )
        {
            $msg = app::get('sysshop')->_("店铺分类名称不能为空");
            throw new \LogicException($msg);
        }

        if( mb_strlen(trim($catName),'utf8') > 20 )
        {
            $msg = app::get('sysshop')->_("店铺分类名称不能超过20个字");
            throw new \LogicException($msg);
        }
    ...
    public function storeCat($data,$shopId)
    {
        ...
        try
        {
            ...
            {
                throw new \LogicException('批量删除删除店铺分类失败');
            }

            if( $data['up'] && !$this->updateShopCat($data['up'],$shopId) )
            {
                throw new \LogicException('更新店铺分类数据失败');
            }

            if( $data['new'] && !$this->addShopCat($data['new'],$shopId) )
            {
                throw new \LogicException('存储店铺分类数据失败');
            }
            $db->commit();
