...
class tags_ctl_admin_tags extends desktop_controller{
	var $url = '?app=tags&ctl=admin_tags';

    public function index(){
    	$url_params = '&wg='.urlencode($_GET['wg']).'&type='.urlencode($_GET['type']).'&app_id='.urlencode($_GET['app_id']);
        $params = array(
            'title'   => app::get('tags')->_('标签管理'),
            'actions' => array(
                array(
                    'label' => app::get('tags')->_('新建普通标签'),
                    'icon'  => 'add.gif',
                    'href'  => $this->url . '&act=tag_add' . $url_params,'target'=>'dialog::{title:\''.app::get('tags')->_('新建普通标签').'\'}'
                ),
            ),
            'base_filter'=>array(
                'tag_type'=>$_GET['type']
    ...
    public function tag_edit( $id ){
       $this->url = '?app='.$_GET['app'].'&ctl='.$_GET['ctl'].'&app_id='.$_GET['app_id'];
       $mdl_tag = app::get('desktop')->model('tag');
       $tag = $mdl_tag->dump($id,'*');
       $pagedata['data'] = $tag;
       return $this->page('tags/edittag.html', $pagedata);
    }
